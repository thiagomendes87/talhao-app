      ...
      <div className="flex flex-col justify-center px-12 py-8 bg-white overflow-y-auto">
        ...
        {emailEnviado ? (
          <div className="text-center space-y-4">
            ...
            <Link href="/entrar" className="block text-[#1f5230] font-semibold text-sm hover:underline">
              Já confirmei → Entrar
            </Link>
          </div>
        ) : (
        ...
        <p className="text-sm text-gray-600 mb-5">
          Já tem conta? <Link href="/entrar" className="text-[#1f5230] font-semibold">Entrar</Link>
        </p>

        {/* Login Social */}
        <div className="mb-5">
          <button onClick={handleGoogle} className="w-full flex items-center justify-center gap-3 border border-[rgba(22,33,19,0.15)] rounded-xl py-3 text-sm font-medium text-gray-800 hover:bg-gray-50 transition-colors">
        ...
        <form onSubmit={handleSignup} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-2 mt-1">
              {perfis.map((p, i) => (
                <button key={p.nome} type="button" onClick={() => setPerfilIdx(i)}
                  className={`border-2 rounded-xl p-3 text-left transition-colors ${perfilIdx === i ? 'border-[#1f5230] bg-[#D8F3DC]/40' : 'border-gray-200 hover:border-gray-300'}`}>
          ...
          <p className="text-center text-xs text-gray-400">
            Ao criar conta você concorda com os <Link href="#" className="text-[#1f5230]">Termos de Uso</Link> e <Link href="#" className="text-[#1f5230]">Política de Privacidade</Link>.
          </p>
        </form>
        </>
        )}
