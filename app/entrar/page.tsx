...
function LoginForm() {
  ...
  return (
    <div className="min-h-screen flex flex-col sm:grid sm:grid-cols-2">
      ...
      <div className="hidden sm:flex flex-col justify-between p-12" style={{ background: 'linear-gradient(145deg, #1B4332 0%, #2D6A4F 60%, #40916C 100%)' }}>
      ...
      <div className="flex flex-col justify-center px-6 sm:px-12 py-10 bg-white min-h-screen sm:min-h-0">
        ...
        <p className="text-sm text-gray-600 mb-8">
          Não tem uma conta?{' '}
          <Link href="/cadastro" className="text-[#1f5230] font-semibold">Cadastre-se grátis</Link>
        </p>

        {/* Login Social */}
        <div className="mb-5">
          <button type="button" onClick={async () => {
              const callbackUrl = buildAuthCallbackUrl(safeRedirectTo)
            ...
            className="w-full flex items-center justify-center gap-3 border border-[rgba(22,33,19,0.15)] rounded-xl py-3 text-sm font-medium text-gray-800 hover:bg-gray-50 transition-colors">
        ...
        <p className="text-center text-sm text-gray-600 mt-6">
          Não tem conta?{' '}
          <Link href="/cadastro" className="text-[#1f5230] font-semibold">Criar conta grátis</Link>
        </p>
      </div>
    </div>
  )
