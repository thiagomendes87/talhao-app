...
export default function LeadCaptureSheet({
  ...
  const handleSubmit = () => {
    window.open('/fazenda-tres-barras.html', '_blank', 'noopener,noreferrer')
    const msg = encodeURIComponent(
      `Oi! Acessei o case da Fazenda Três Barras na Talhão e quero uma análise da minha fazenda "${fazenda || 'minha fazenda'}". Me chamo ${nome}. Meu WhatsApp: ${whatsapp}.`
    )
      ...
      <div className="relative mx-auto max-w-5xl rounded-t-3xl border border-b-0 border-[rgba(22,33,19,0.12)] bg-white px-6 py-6 shadow-[0_-12px_48px_rgba(22,33,19,0.14)] sm:px-10 sm:py-8">
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2 lg:gap-10">
          <div className="flex flex-col justify-between gap-4">
            ...
            <div className="relative inline-flex items-center gap-2 cursor-not-allowed select-none">
              <span
                className="text-sm font-semibold text-[#2D6A4F]"
                style={{ filter: 'blur(3.5px)', userSelect: 'none', pointerEvents: 'none' }}
              >
                Ver relatório completo →
              </span>
              <span className="text-base" title="Preencha seus dados para acessar">
                🔒
              </span>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-4">
            ...
            <p className="text-center text-xs text-gray-400">Acesso imediato · Sem cadastro</p>
          </div>
        </div>
      </div>
    </div>
